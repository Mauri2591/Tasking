...
class Clientes extends Conexion
{
    ...
    {
        $conn = parent::get_conexion();
        $sql = "INSERT INTO clientes (client_rs, pais_id, client_cuit, client_correo, client_tel, est) VALUES (?,?,?,?,?,1)";
        $stmt = $conn->prepare($sql);
        $stmt->bindValue(1, htmlspecialchars($client_rs, ENT_QUOTES), PDO::PARAM_STR);
        $stmt->bindValue(2, htmlspecialchars($pais_id, ENT_QUOTES), PDO::PARAM_INT);
        $stmt->bindValue(3, htmlspecialchars($client_cuit, ENT_QUOTES), PDO::PARAM_STR);
        $stmt->bindValue(4, htmlspecialchars($client_correo, ENT_QUOTES), PDO::PARAM_STR);
        $stmt->bindValue(5, htmlspecialchars($client_tel, ENT_QUOTES), PDO::PARAM_STR);
        $stmt->execute();
    }
