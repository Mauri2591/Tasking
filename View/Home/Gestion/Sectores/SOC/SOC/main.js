$(document).ready(function () {
    tabla = $("#table_proyectos_nuevos_eh_soc").dataTable({
        "aProcessing": true,
        "aServerSide": true,
        dom: 'Bfrtip',
        "searching": true,
        ...
        "ajax": {
            url: "../../../../../../Controller/ctrProyectos.php?proy=get_proyectos_soc",
            type: "post",
            dataType: "json",
            data: {
                sector_id: 2, // soc
    ...
    tabla = $("#table_proyectos_abiertos_eh_soc").dataTable({
        "aProcessing": true,
        "aServerSide": true,
        dom: 'Bfrtip',
        "searching": true,
    ...
    tabla = $("#table_proyectos_realizados_eh_soc").dataTable({
        "aProcessing": true,
        "aServerSide": true,
        dom: 'Bfrtip',
        "searching": true,
...
function cambiar_a_borrador(id_proyecto_cantidad_servicios) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $('#table_proyectos_abiertos_eh_soc').DataTable().ajax.reload(null, false);
            $('#table_proyectos_nuevos_eh_soc').DataTable().ajax.reload(null, false);
            Swal.fire({
                icon: "success",
                title: "Bien",
...
function cambiar_a_realizado(id_proyecto_cantidad_servicios) {
    Swal.fire({
        ...
        if (result.isConfirmed) {
            ...
            $('#table_proyectos_abiertos_eh_soc').DataTable().ajax.reload(null, false);
            $('#table_proyectos_realizados_eh_soc').DataTable().ajax.reload(null, false);
        }
    })
}